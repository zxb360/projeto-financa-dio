        ...
        <form onSubmit={handleSubmit(onSubmit)} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="space-y-4">
            <label className="block">
              ...
              <input className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" {...register('name')} />
              {errors.name && <p className="mt-1 text-sm text-rose-600">{errors.name.message}</p>}
            </label>
            <label className="block">
              <span className="text-sm font-medium text-slate-700">Valor desejado</span>
              <input type="number" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" {...register('targetAmount')} />
              {errors.targetAmount && <p className="mt-1 text-sm text-rose-600">{errors.targetAmount.message}</p>}
            </label>
            <label className="block">
              <span className="text-sm font-medium text-slate-700">Prazo em meses</span>
              <input type="number" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" {...register('months')} />
        ...
        {profile.goals.length === 0 ? (
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {profile.goals.map((goal) => (
              <article key={goal.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <p className="text-sm font-medium text-slate-500">{goal.name}</p>
                <p className="mt-2 text-2xl font-bold text-slate-950">{formatCurrency(goal.targetAmount)}</p>
